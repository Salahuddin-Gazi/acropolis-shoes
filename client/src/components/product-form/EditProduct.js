...
const EditProduct = ({ updateProduct, setAlert, product: { product, loading } }) => {
  const Navigate = useNavigate();
  let { prd_id } = useParams();
  // Using React Hook
  // Creating existing fields
  const [formData, setFromData] = useState({
    status: product ? product.status : "",
    itemName: product ? product.itemName : "",
    itemBrand: product ? product.itemBrand : "",
    itemDescription: product ? product.itemDescription : "",
    itemPrice: product ? product.itemPrice : "",
    itemQuantity: product ? product.itemQuantity : "",
    itemColors: product ? product.itemColors.map((color) => color + " ").toString() : [],
    itemCategory: product ? product.itemCategory : "",
    descriptionImages: product ? product.itemImages.descriptionImages : [],
    mainImage: product ? product.itemImages.mainImage : "",
  });
  ...
  const onSubmit = async (e) => {
    ...
    // console.log(formData);
    if (mainImage) {
      updateProduct(formData, prd_id);
    } else {
      setAlert("Fill the required fields", "warning");
    }
  };
  useEffect(() => {
    !product && Navigate("/", { replace: true });
  }, [product, Navigate]);
  return loading ? (
    <Spinner />
  ) : (
    <Fragment>
      ...
      <h1 className="large text-primary">Edit Product, Admin</h1>
      <form className="form" onSubmit={(e) => onSubmit(e)}>
        ...
        <div className="form-group col-md-8 pb-3 input-group-lg">
          <input type="text" className="form-control" placeholder="Barand Name" name="itemBrand" value={itemBrand} required onChange={(e) => onChange(e)} />
                ...
                <FileBase64
                  ...
                  onDone={(file) => {
                    ...
                    if (file.type.slice(0, 5) === "image") {
                      ...
                      // setFromData({ ...formData, descriptionImages: [] });
                      setAlert("Select Only Image", "warning");
                    }
                  }}
                />
...
export default connect(mapStateToProps, { updateProduct, setAlert })(EditProduct);
